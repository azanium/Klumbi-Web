                    ...
                    <?php
                        if($listconfig)
                        {
                            ...
                            foreach($listconfig as $dt)
                            {
                                ...
                                $datanilai .= $this->mongo_db->count2(array('mix_id'=>(string)$dt['_id'],'type'=>'AvatarMixlove')).", ";
                            }
                            echo $datanilai;
                        }
                        else
                                ...
                                <div class="accordion-body">
                                    <table id="datatable" class="table table-striped table-bordered datatables datatable_rd" cellpadding="0" cellspacing="0" border="0">
                                        <thead>
                                            <tr>
                                                <th width="5%">No</th>
                                                <th width="75%">Avatar Mix Name</th>
                                                <th width="10%">Love</th>
                                        ...
                                        <tbody>
                                            ...
                                            if($listconfig)
                                            {
                                                ...
                                                foreach($listconfig as $dt)
                                                {
                                                    $this->mongo_db->select_db("Social");
                                                    $this->mongo_db->select_collection("Social");
                                                    $totallike = (int)$this->mongo_db->count(array('mix_id'=>(string)$dt['_id'],'type'=>'AvatarMixlove'));  
                                                    $this->mongo_db->select_collection("Social");
                                                    $totalcomment = (int)$this->mongo_db->count(array('mix_id'=>(string)$dt['_id'],'type'=>'AvatarMixComment')); 
                                                    echo "<tr>";
                                                    echo "<td>".$i."</td>";
                                                    echo "<td>".$dt['name']."</td>";
