...
$(document).ready(function() {
   ...
   var colors = Highcharts.getOptions().colors,
                ...
                <?php
                    if($listbanner)
                    {
                        ...
                        foreach($listbanner as $dt)
                        {
                            ...
                            $text .="{";
                            $text .="y:".$this->mongo_db->count2(array('banner_id'=>(string)$dt['_id'],'type'=>'BannerLove')).",";                            
                            ...
                            $texttgl = "";
                            $textvalue = "";
                            if($this->mongo_db->count2(array("banner_id"=>(string)$dt['_id'])))
                            {
                                $firstdate = $this->mongo_db->find(array("banner_id"=>(string)$dt['_id']),0,1,array('datetime'=>-1));
                                $lastdate = $this->mongo_db->find(array("banner_id"=>(string)$dt['_id']),0,1,array('datetime'=>1));
        ...
        chart = new Highcharts.Chart({
            ...
            title: {
                text: 'Reporting Banner'
            },
            subtitle: {
                text: 'Click the columns to view comments. Click again to view banner.'
            },
            xAxis: {
            ...
            tooltip: {
                formatter: function() {
                    ...
                    if (point.drilldown) {
                        ...
                        s += 'Click to back to list banner';
                    }
                    return s;
                }
            },
                                        ...
                                        <tbody>
                                            ...
                                            if($listbanner)
                                            {
                                                ...
                                                foreach($listbanner as $dt)
                                                {
                                                    $this->mongo_db->select_db("Social");
                                                    $this->mongo_db->select_collection("Social");
                                                    $totallike = (int)$this->mongo_db->count(array('banner_id'=>(string)$dt['_id'],'type'=>'BannerLove'));  
                                                    $this->mongo_db->select_collection("Social");
                                                    $totalcomment = (int)$this->mongo_db->count(array('banner_id'=>(string)$dt['_id'],'type'=>'BannerComment')); 
                                                    echo "<tr>";
                                                    echo "<td>".$i."</td>";
                                                    echo "<td>".$dt['name']."</td>";
