...
class Type extends CI_Controller 
{
    public function __construct() {
        ...
        $this->m_checking->module("Content Type","module9",FALSE,TRUE,"home");
    }
    function index()
    {
        $css=array(
        ...
        $this->template_admin->header_web(TRUE,"Content Type",$css,$js,TRUE,"");
    ...
    function list_data()
    {
        ...
        foreach($data as $dt)
        {
            ...
            if($this->m_checking->actions("Content Type","module9","Delete",TRUE,FALSE,"home"))
            {
	...
	if(IS_AJAX)
        ...
        {
            redirect('content/type/index'); 
        }
    }
    function cruid_type()
    {        
        $this->form_validation->set_rules('name','Type Name','trim|required|htmlspecialchars|xss_clean');
        ...
        else
        {
            ...
            if($id=='')
            {
                $this->m_checking->actions("Content Type","module9","Add",FALSE,FALSE,"home");
                $this->mongo_db->select_db("Articles");
                $this->mongo_db->select_collection("ContentType");
                $level_id=$this->mongo_db->insert($datatinsert);
                $this->m_user->tulis_log("Add Content Type",$url,$user);
                $output['message'] = "<i class='success'>New Data is added</i>";
            }
            else
            {
                $this->m_checking->actions("Content Type","module9","Edit",FALSE,FALSE,"home");
                $this->mongo_db->select_db("Articles");
                $this->mongo_db->select_collection("ContentType");
                $this->mongo_db->update(array('_id'=> $this->mongo_db->mongoid($id)),array('$set'=>$datatinsert));
                $this->m_user->tulis_log("Update Content Type",$url,$user);
    ...
    function delete($id="")
    {
        $this->m_checking->actions("Content Type","module9","Delete",FALSE,FALSE,"home");
        $this->mongo_db->select_db("Articles");
        $this->mongo_db->select_collection("ContentType");
        $this->mongo_db->remove(array('_id' => $this->mongo_db->mongoid($id)));
        $url = current_url();
        $user = $this->session->userdata('username');
        $this->m_user->tulis_log("Delete Content Type",$url,$user);
        $output = array(
