...
class Category extends CI_Controller 
{
    public function __construct() {
        ...
        $this->m_checking->module("Content Category","module9",FALSE,TRUE,"home");
    }
    function index()
    {
        $css=array(
        ...
        $this->template_admin->header_web(TRUE,"Business Category",$css,$js,TRUE,"");
        ...
        $this->load->view("listcategory_view");
    ...
    function list_data()
    {
        ...
        foreach($data as $dt)
        {
            ...
            if($this->m_checking->actions("Content Category","module9","Delete",TRUE,FALSE,"home"))
            {
    ...
    function cruid_category()
    {        
        $this->form_validation->set_rules('name','Category Name','trim|required|htmlspecialchars|xss_clean');
        $this->form_validation->set_rules('txtdesc','Descriptions','trim|required|htmlspecialchars|xss_clean');
        ...
        else
        {
            ...
            if($id=='')
            {
                $this->m_checking->actions("Content Category","module9","Add",FALSE,FALSE,"home");
                $this->mongo_db->select_db("Articles");
                $this->mongo_db->select_collection("ContentCategory");
                $level_id=$this->mongo_db->insert($datatinsert);
                $this->m_user->tulis_log("Add Business Category",$url,$user);
                $output['message'] = "<i class='success'>New Data is added</i>";
            }
            else
            {
                $this->m_checking->actions("Content Category","module9","Edit",FALSE,FALSE,"home");
                $this->mongo_db->select_db("Articles");
                $this->mongo_db->select_collection("ContentCategory");
                $this->mongo_db->update(array('_id'=> $this->mongo_db->mongoid($id)),array('$set'=>$datatinsert));
                $this->m_user->tulis_log("Update Business Category",$url,$user);
    ...
    function delete($id="")
    {
        $this->m_checking->actions("Content Category","module9","Delete",FALSE,FALSE,"home");
        $this->mongo_db->select_db("Articles");
        $this->mongo_db->select_collection("ContentCategory");
        $this->mongo_db->remove(array('_id' => $this->mongo_db->mongoid($id)));
        $url = current_url();
        $user = $this->session->userdata('username');
        $this->m_user->tulis_log("Delete Content Category",$url,$user);
        $output = array(
