...
class Content extends CI_Controller 
{
    public function __construct() {
        ...
        $this->m_checking->module("Content Page","module9",FALSE,TRUE,"home");
    }
    function index()
    {
        $data['txtid']='';
        ...
        $this->template_admin->header_web(TRUE,"Pages &amp; Contents",$css,$js,TRUE,"");
    ...
    function list_data()
    {
        ...
        foreach($data as $dt)
        {
            if($this->m_checking->actions("Content Page","module9","Edit",TRUE,FALSE,"home"))
            {
                $url=$this->template_admin->link("content/detail/".$dt['_id']);
                $detail=$this->template_icon->detail_onclick("list_detail('".$url."','','html')",$url,'Edit',"pencil.png","","linkdetail");
            }
            if($this->m_checking->actions("Content Page","module9","Delete",TRUE,FALSE,"home"))
            {
                $delete=$this->template_icon->detail_onclick("hapusdata('".$dt['_id']."','Are you sure want to delete Content ".$dt['title']."')","",'Delete',"delete.png","","linkdelete");
            }
            $url_link = $this->template_admin->link("home/content/index/".  urlencode($dt['alias']));
    ...
    {        
        ...
        else
        {
            ...
            if($id=='')
            {
                $this->m_checking->actions("Content Page","module9","Add",FALSE,FALSE,"home");
                $this->mongo_db->select_db("Articles");
                $this->mongo_db->select_collection("ContentPage");
                ...
                $this->m_user->tulis_log("Add Content",$url,$user);
            }
            else
            {
                $this->m_checking->actions("Content Page","module9","Edit",FALSE,FALSE,"home");
                $this->mongo_db->select_db("Articles");
                $this->mongo_db->select_collection("ContentPage");
                $this->mongo_db->update(array('_id'=> $this->mongo_db->mongoid($id)),array('$set'=>$datatinsert));
                $this->m_user->tulis_log("Update Content",$url,$user);
            }  
        }
        if(IS_AJAX)
        {            
    ...
    function detail($id="")
    {
        ...
        $this->load->view("form_content",$data);
    }
    function delete($id="")
    {
        $this->m_checking->actions("Content Page","module9","Delete",FALSE,FALSE,"home");
        $this->mongo_db->select_db("Articles");
        $this->mongo_db->select_collection("ContentPage");
        $this->mongo_db->remove(array('_id' => $this->mongo_db->mongoid($id)));
        $url = current_url();
        $user = $this->session->userdata('username');
        $this->m_user->tulis_log("Delete Content",$url,$user);
        $output = array(
