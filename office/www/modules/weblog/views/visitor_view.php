...
$(document).ready(function() {
    dttable=$('#datatable').dataTable( {
        ...
        "bProcessing": true,
        "bServerSide": true,
        "sAjaxSource": "<?php echo $this->template_admin->link("weblog/visitor_count/list_data"); ?>",
    ...
    $('#truncatelog').click(function(){
        BootstrapDialog.confirm("Are You sure want to remove all Log Visitor ?", function(result){
            if(result) 
            {
                var url='<?php echo $this->template_admin->link("weblog/visitor_count/truncate"); ?>';
                $.ajax({
                    type: "POST",
                    url: url,
                    data:"",
...
function hapusdata(id,pesan)
{
    BootstrapDialog.confirm(pesan, function(result){
        if(result) 
        {
            var url='<?php echo base_url(); ?>weblog/visitor_count/delete/'+id;
            $.ajax({
                type: "POST",
                url: url,
                data:"",
            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body collapse in">
                    <p align="right">
                        ...
                        {
                            echo '<a id="truncatelog" class="btn btn-sm btn-danger"><i class="icon-trash"></i> Delete All Visitor Logs</a>&nbsp;&nbsp;'; 
                        }
                        ?>
