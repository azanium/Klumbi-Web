...
class Avataritems extends CI_Controller 
{
    ...
    {
        $this->m_checking->actions("Avatar","module6","View Likes",FALSE,TRUE,"avatar/avatar/index");
        ...
        $this->template_admin->header_web(TRUE,"Avatar Items Love",$css,$js,TRUE,"");
        ...
        $this->load->view("avataritemslike_view",$add_properties);
    ...
    function list_itemdata($idavatar="")
    {
        $this->m_checking->actions("Avatar","module6","View Likes",FALSE,TRUE,"avatar/avatar/index");
        ...
        $pencarian=array('avatar_id'=>$idavatar,'type'=>'AvatarItemLove');
        ...
        {
            redirect('avatar/avataritems/datalike/'.$idavatar); 
        }
    }
    function datacomments($idavatar="")
    {
        $this->m_checking->actions("Avatar","module6","View Comments",FALSE,TRUE,"avatar/avatar/index");
        $add_properties['id_avatar']=$idavatar;
        $css=array(
            base_url()."resources/css/jqueryui.css",
            base_url()."resources/plugin/codeprettifier/prettify.css",
        ...
        $this->template_admin->header_web(TRUE,"Avatar Item Comments",$css,$js,TRUE,"");
        $this->template_admin->headerbar(); 
        $this->template_admin->top_menu(TRUE);
        $this->load->view("panel/panel_utama");
        $this->template_admin->panel_menu();
        $this->load->view("panel/panel_content");
        $this->load->view("panel/panel_wrap");
        $this->load->view("avataritemscomment_view",$add_properties);
        $this->load->view("panel/close_div");
        $this->load->view("panel/close_div");
        $this->template_admin->addresbar();
        $this->load->view("panel/close_div");
    ...
    function list_itemdatacommet($idavatar="")
    {
        ...
        $limit=(isset($_GET['iDisplayLength']))?(int)$_GET['iDisplayLength']:10;
        $sEcho=(isset($_GET['sEcho']))?(int)$_GET['sEcho']:1;
        $pencarian=array('avatar_id'=>$idavatar,'type'=>'AvatarItemComment');
        ...
        foreach($data as $dt)
        {
            ...
            if($this->m_checking->actions("Avatar","module6","Delete Comments",TRUE,FALSE,"avatar/avatar/index"))
            {
        ...
        {
            redirect('avatar/avataritems/datacomments/'.$idavatar); 
        }
    }
    function delete($id="")
    {
        $this->m_checking->actions("Avatar","module6","Delete Comments",FALSE,FALSE,"avatar/avatar/index");
        $this->mongo_db->select_db("Social");
        $this->mongo_db->select_collection("Social");
        $this->mongo_db->remove(array('_id' => $this->mongo_db->mongoid($id)));
        $url = current_url();
        $user = $this->session->userdata('username');
        $this->m_user->tulis_log("Delete Comment on Avatar Items",$url,$user);
        $output = array(
