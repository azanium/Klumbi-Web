...
class Mgcolor extends CI_Controller 
{
    public function __construct() {
        parent::__construct();
        $this->load->library(array('form_validation','session','template_admin','template_icon','tambahan_fungsi','cek_session'));
        $this->cek_session->cek_login();
        $this->m_checking->module("Color","module6",FALSE,TRUE,"home/index");
    }
    function index()
    {
        $css=array(
        ...
        $this->template_admin->header_web(TRUE,"Color Manager",$css,$js,TRUE,"");
    ...
    function list_data()
    {
        ...
        foreach($data as $dt)
        {
            ...
            $output['aaData'][] = array(
                ...
                $dt['name'],
                $dt['color'],
                "<span style='width: 15px;height: 15px;display:block;margin:20px 3px 3px 3px;background-color: ".$dt['color'].";'></span>" ."<br />#". dechex($tempwarnargb[0]).dechex($tempwarnargb[1]).dechex($tempwarnargb[2]),
                $img,
                $detail.$delete,
            );
            $i++;           
        ...
        {
            redirect('inventory/mgcolor/index'); 
        }
    }
    function cruid_color()
    {        
        $this->form_validation->set_rules('name','Name','trim|required|htmlspecialchars|xss_clean');
        $this->form_validation->set_rules('txtskincolor','Color','trim|htmlspecialchars|xss_clean');
        ...
        else
        {
            ...
            if($id=='')
            {
                $this->m_checking->actions("Color","module6","Add",FALSE,TRUE,"home/index");
                $this->mongo_db->select_db("Assets");
                $this->mongo_db->select_collection("Color");
                $level_id=$this->mongo_db->insert($datatinsert);
                $this->m_user->tulis_log("Add Color",$url,$user);
                $output['message'] = "<i class='success'>New Data is added</i>";
            }
            else
            {
                $this->m_checking->actions("Color","module6","Edit",FALSE,TRUE,"home/index");
                $this->mongo_db->select_db("Assets");
                $this->mongo_db->select_collection("Color");
                $this->mongo_db->update(array('_id'=> $this->mongo_db->mongoid($id)),array('$set'=>$datatinsert));
                $this->m_user->tulis_log("Update Color",$url,$user);
    ...
    function delete($id="")
    {
        $this->m_checking->actions("Color","module6","Delete",FALSE,FALSE,"home/index");
        $this->mongo_db->select_db("Assets");
        $this->mongo_db->select_collection("Color");
        ...
        $url = current_url();
        $user = $this->session->userdata('username');
        $this->m_user->tulis_log("Delete Color",$url,$user);
        $output = array(
