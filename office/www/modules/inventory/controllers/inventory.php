...
class Inventory extends CI_Controller 
{
    public function __construct() {
        ...
        $this->m_checking->module("Inventory","module6",FALSE,TRUE,"home");
    }
    function index()
    {
        $css=array(
        ...
        $this->template_admin->header_web(TRUE,"Inventory",$css,$js,TRUE,"");
    ...
    function list_data()
    {
        ...
        foreach($data as $dt)
        {
            ...
            $detail="";
            $delete="";
            if($this->m_checking->actions("Inventory","module6","Edit",TRUE,FALSE,"home"))
            {
                $detail=$this->template_icon->detail_onclick("ubahdata('".$dt['_id']."','".$dt['name']."','". $filename ."')","#editdata",'Edit',"pencil.png","","","data-toggle='modal'");
            } 
            if($this->m_checking->actions("Inventory","module6","Delete",TRUE,FALSE,"home"))
            {
                $delete=$this->template_icon->detail_onclick("hapusdata('".$dt['_id']."','Are you sure want to delete Inventory ".$dt['name']."')","",'Delete',"delete.png","","linkdelete");
            } 
            $output['aaData'][] = array(
                $i,
                $dt['name'],
    ...
    {        
        ...
        else
        {
            ...
            if($id=='')
            {
                $this->m_checking->actions("Inventory","module6","Add",FALSE,TRUE,"home");
                $this->mongo_db->select_db("Assets");
                $this->mongo_db->select_collection("Inventory");
                $level_id=$this->mongo_db->insert($datatinsert);
                $this->m_user->tulis_log("Add Inventory",$url,$user);
                $output['message'] = "<i class='success'>New Data is added</i>";
            }
            else
            {
                $this->m_checking->actions("Inventory","module6","Edit",FALSE,TRUE,"home");
                $this->mongo_db->select_db("Assets");
                $this->mongo_db->select_collection("Inventory");
                $this->mongo_db->update(array('_id'=> $this->mongo_db->mongoid($id)),array('$set'=>$datatinsert));
                $this->m_user->tulis_log("Update Inventory",$url,$user);
    ...
    function delete($id="")
    {
        $this->m_checking->actions("Inventory","module6","Delete",FALSE,FALSE,"home");
        $this->mongo_db->select_db("Assets");
        $this->mongo_db->select_collection("Inventory");
        ...
        $url = current_url();
        $user = $this->session->userdata('username');
        $this->m_user->tulis_log("Delete Inventory",$url,$user);
        $output = array(
