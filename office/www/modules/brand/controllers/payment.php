...
class Payment extends CI_Controller 
{
    public function __construct() {
        ...
        $this->m_checking->module("Payment Method","module5",FALSE,TRUE,"home");
    }
    function index()
    {
        $css=array(
        ...
        $this->template_admin->header_web(TRUE,"Payment Methode",$css,$js,TRUE,"");
        ...
        $this->load->view("listpayment_view");
    ...
    function list_data()
    {
        ...
        foreach($data as $dt)
        {
            ...
            {
                $detail=$this->template_icon->detail_onclick("ubahdata('".$dt['_id']."','".$dt['name']."')","#editdata",'Edit',"pencil.png","","","data-toggle='modal'");
            }
            if($this->m_checking->actions("Payment Method","module5","Delete",TRUE,FALSE,"home"))
            {
                $delete=$this->template_icon->detail_onclick("hapusdata('".$dt['_id']."','Are you sure want to delete Payment ".$dt['name']."')","",'Delete',"delete.png","","linkdelete");
            }
            $output['aaData'][] = array(
                $i,
                $dt['name'],
	...
	if(IS_AJAX)
        ...
        {
            redirect('brand/payment/index'); 
        }
    }
    function cruid_payment()
    {        
        $this->form_validation->set_rules('name','Payment Name','trim|required|htmlspecialchars|xss_clean');
        ...
        else
        {
            ...
            if($id=='')
            {
                $this->m_checking->actions("Payment Method","module5","Add",FALSE,FALSE,"home");
                $this->mongo_db->select_db("Assets");
                $this->mongo_db->select_collection("Payment");
                $this->mongo_db->insert($datatinsert);
                $this->m_user->tulis_log("Add Payment",$url,$user);
                $output['message'] = "<i class='success'>New Data is added</i>";
            }
            else
            {
                $this->m_checking->actions("Payment Method","module5","Edit",FALSE,FALSE,"home");
                $this->mongo_db->select_db("Assets");
                $this->mongo_db->select_collection("Payment");
                $namalama=$this->mongo_db->findOne(array('_id'=> $this->mongo_db->mongoid($id)));
                $this->mongo_db->select_collection("Avatar");
                $this->mongo_db->update(array('payment'=> $namalama['name']),array('$set'=>array('payment'=>$name)));
                $this->mongo_db->select_collection("Payment");
                $this->mongo_db->update(array('_id'=> $this->mongo_db->mongoid($id)),array('$set'=>$datatinsert));
                $this->m_user->tulis_log("Update Payment",$url,$user);
    ...
    function delete($id="")
    {
        $this->m_checking->actions("Payment Method","module5","Delete",FALSE,FALSE,"home");
        $this->mongo_db->select_db("Assets");
        $this->mongo_db->select_collection("Payment");
        $this->mongo_db->remove(array('_id' => $this->mongo_db->mongoid($id)));
        $url = current_url();
        $user = $this->session->userdata('username');
        $this->m_user->tulis_log("Delete Payment",$url,$user);
        $output = array(
