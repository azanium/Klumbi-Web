...
$(document).ready(function() {
    dttable=$('#datatable').dataTable( {
        ...
        "bFilter": true,
        "bServerSide": true,
        "sAjaxSource": "<?php echo $this->template_admin->link("management/user/list_data"); ?>",
        ...
        'aoColumns': [ { "sClass" : "alignRight","bSortable": false }, {"bSortable": false }, {"bSortable": false }, {"bSortable": false }, {"bSortable": false }, {"bSortable": false }, {"bSortable": false }, { "sClass" : "text-center","bSortable": false }]
    });
...
function lihatdetail(id,group,brand)
{
    $('#txtid').val(id);
    $('#group').val(group);
    $('#brand').val(brand);
...
function hapusdata(id,pesan)
{
    BootstrapDialog.confirm(pesan, function(result){
        if(result) 
        {
            var url='<?php echo $this->template_admin->link("management/user/cruid_user"); ?>';
            $.ajax({
                    type: "POST",
                    url: url,
                    data:{txtid:id,group:'',brand:''},
                    dataType: "json",
                    beforeSend: function ( xhr ) {
                        $("#loadingprocess").html('<div class="alert alert-dismissable alert-warning">' +
						'<strong>Warning!</strong> ' +
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST" id="editbrandfrm" action="<?php echo $this->template_admin->link("management/user/cruid_user"); ?>">
                    <table class="table table-striped table-bordered table-hover" cellpadding="0" cellspacing="0" border="0">
                        <tr>
                            ...
                            <td>
                                <select id='group' name='group' class='form-control'>
                                    ...
                                    <?php
                                    foreach($listgroup as $dt)
                                    { 
                                        echo "<option value='".$dt['Code']."'>".$dt['Name']."</option>";
                                    }
                                    ?>
                                </select>
                            </td>
                        ...
                        <tr>
                            <td><label for="brand">Store Name</label></td>
                            <td>
