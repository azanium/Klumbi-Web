...
function hapusquest(pertanyaan,id,idquest)
{
    BootstrapDialog.confirm(pertanyaan, function(result){
        if(result) 
        {
            var url='<?php echo base_url(); ?>quest/report/delete/'+id+'/'+idquest;
            $.ajax({
                type: "POST",
                url: url,
                data:"",
                ...
                <table id="chiledatatable">
                    ...
                    <tbody>
                        ...
                        foreach($isitable as $key)
                        {
                            ...
                            if($jns==TRUE)
                            {
                                $imgdelete=$this->template_icon->detail_onclick("hapusquest('Are you sure want to undone Player ".$key['username']." for this quest ?','".(string)$key['_id']."','".$id_quest."')","",'Undone',"delete.png","","linkdelete");
