...
class Following extends CI_Controller 
{
    public function __construct() {
        ...
        $this->m_checking->actions("User","module2","View Following",FALSE,TRUE,"home");
    }
    ...
    {        
        ...
        $this->template_admin->header_web(TRUE,"View User Following",$css,$js,TRUE,"");
        ...
        $this->load->view("userfollowing_view",$datapage);
    ...
    function list_data($iduser="")
    {
        ...
        $pencarian=array('friend_id'=>$iduser,'type'=>'Follower');
        ...
        foreach($data as $dt)
        {
            ...
            $output['aaData'][] = array(
                $i,
                "<a href='#' onclick='lihatdetail(\"".$dt['user_id']."\");return false;'>".$email." / ".$username."</a>",
                $tgl,
            );
            $i++;           
        }  
	if(IS_AJAX)
        ...
        {
            redirect('user/following/index/'.$iduser); 
        }
    }
}